...
exports.tulultOlnoorKhadgalya = asyncHandler(async (req, res, next) => {
  ...
  try {
    var aldaaniiMsg;
    for (const tulbur of guilgeenuud) {
  ...
  try {
    ...
    if (kholboltuud) {
      for (const kholbolt of kholboltuud) {
        var dansnuud = await Dans(kholbolt)
          .find({
        ...
        for (const dans of dansnuud) {
          if (!!dans.bank) {
            ...
            var guilgeenuudQPAY = await BankniiGuilgee(kholbolt, true)
              .find(match1)
              .lean();
            var match2 = match;
            const qpayRegexNot = {
            ...
            var guilgeenuudNOTQPAY = await BankniiGuilgee(kholbolt, true)
              .find(match2)
              .lean();
            var khaikhNukhtsul;
            var tailbar = [];
            ...
            if (guilgeenuudQPAY?.length > 0) {
              guilgeenuudQPAY.forEach(async (x) => {
                ...
                var oldsonGereenuud = await Geree(kholbolt, true)
                  .find({
            ...
            if (guilgeenuudNOTQPAY?.length > 0) {
              guilgeenuudNOTQPAY.forEach(async (x) => {
                ...
                if (x.relatedAccount != null) {
                  ...
                  var oldsonGereenuudCtAcntOrg = await Geree(kholbolt, true)
                    .find({
                ...
                var oldsonGereenuudUtas = await Geree(kholbolt, true)
                  .find({
                    utas: { $in: tailbar },
                    tuluv: 1,
                    barilgiinId: x.barilgiinId,
                ...
                var oldsonGereenuudRegister = await Geree(kholbolt, true)
                  .find({
                    register: { $in: tailbar },
                    tuluv: 1,
                    barilgiinId: x.barilgiinId,
  ...
  try {
    ...
    let query = {
      $or: [
        ...
        { "barilguud.tokhirgoo.aldangiGereeTusBur": true },
      ],
    };
    if (!!baiguullagiinId) {
      var ObjectId = require("mongodb").ObjectId;
    ...
    for (const baiguullaga of baiguullaguud) {
      ...
      for (const barilga of baiguullaga.barilguud) {
        ...
        let aldangiBodojEkhlekhOgnoo = barilga?.tokhirgoo?.aldangiOgnoo
          ...
          : null;
        const niitGereenuud = await Geree(kholbolt, true).aggregate([
          {
            $match: {
        ...
        for (const gereeData of niitGereenuud) {
          if (barilga?.tokhirgoo?.aldangiGereeTusBur) {
            ...
            aldangiChuluulukhKhonog =
              gereeData._id.aldangiChuluulukhKhonog || 0;
  ...
  try {
    ...
    if (req.body.guilgeeniiId) {
      const archiveBeforeDate = new Date(
        tuxainBaritsaa?.ognoo || tuxainGuilgee?.ognoo,
      );
      archiveBeforeDate.setHours(0, 0, 0, 0);
      const y = archiveBeforeDate.getFullYear();
      const m = archiveBeforeDate.getMonth() + 1;
...
exports.uldegdelBodyo = asyncHandler(async (req, res, next) => {
  ...
  Geree(req.body.tukhainBaaziinKholbolt, true)
    .aggregate(query)
    .then(async (result) => {
      const tureesiinUldegdel = parseFloat((result[0]?.tureesiinUldegdel || 0).toFixed(2));
      ...
      const aldangiinUldegdel = parseFloat((geree?.aldangiinUldegdel || 0).toFixed(2));
      const baritsaaniiUldegdel = parseFloat((geree?.baritsaaniiUldegdel || 0).toFixed(2));

     let niitTulsunAldangiMatch = { "avlaga.guilgeenuud.turul": "aldangi" };
      if (req.body.ognoo && req.body.ognoo[0] && req.body.ognoo[1]) {
        niitTulsunAldangiMatch["avlaga.guilgeenuud.ognoo"] = {
          $gte: new Date(req.body.ognoo[0]),
          $lte: new Date(req.body.ognoo[1]),
        };
      }
      const aldangiAggResult = await Geree(req.body.tukhainBaaziinKholbolt, true).aggregate([
        {
          $match: {
            gereeniiDugaar: req.body.gereeniiDugaar,
            baiguullagiinId: req.body.baiguullagiinId,
        ...
        { $unwind: "$avlaga.guilgeenuud" },
        { $match: niitTulsunAldangiMatch },
        {
          $group: {
            _id: null,
            niitTulsunAldangi: { $sum: { $ifNull: ["$avlaga.guilgeenuud.tulsunAldangi", 0] } },
          },
        },
      ]);
      const niitTulsunAldangi = parseFloat((aldangiAggResult[0]?.niitTulsunAldangi || 0).toFixed(2));
...
exports.khungulultUstgaya = asyncHandler(async (req, res, next) => {
  try {
    ...
    try {
      ...
      for (const gereeniiDugaar of gereeniiDugaaruud) {
        khyamdraluud = [];
...
exports.tukhainOgnoogoorZardalBodojOruulya = asyncHandler(
  async (req, res, next) => {
    try {
      ...
      var khariu = [];
      if (gereenuud)
        for (const element of gereenuud) {
...
exports.qpayGuilgeeGereeOnooyo = asyncHandler(async (req, res, next) => {
  try {
    ...
    var khaikhNukhtsul;
    for (const x of qpayGuilgeenuud) {
      khaikhNukhtsul = [];
      var tailbar;
      if (x.description) tailbar = x.description.split(/,| /);
      else if (x.TxAddInf) tailbar = x.TxAddInf.split(/,| /);
...
exports.tukhainOgnoogoorBukhAvlagaBodojOruulya = asyncHandler(
  async (req, res, next) => {
    try {
      ...
      if (gereenuud)
        for (const element of ajillakhGereenuud) {
          object = {
            tulukhDun: element.sariinTurees,
            undsenDun: element.sariinTurees,
            ognoo: moment(req.body.duusakhOgnoo).set(
...
exports.gereenuudedZalruulgaOruulya = asyncHandler(async (req, res, next) => {
  try {
    ...
    if (objectuud)
      for (const element of objectuud) {
...
exports.fcZasvarKhiie = asyncHandler(async (req, res, next) => {
  try {
    ...
    if (gereenuud)
      for (const geree of gereenuud) {
        for (const guilgee of geree?.niitTulbur) {
...
exports.avlagaZasay = asyncHandler(async (req, res, next) => {
  try {
    ...
    var tempData = await BankniiGuilgee(
      req.body.tukhainBaaziinKholbolt,
      true,
    ).find(match);
...
exports.ashiglakhKhonogTootsoolokh = asyncHandler(async (req, res, next) => {
  try {
    ...
    var filteredAvlagas = filteredGeree?.filter(
      (e) =>
        e.tulukhDun > 0 && (e.turul === "khuvaari" || e.turul === "avlaga"),
    );
