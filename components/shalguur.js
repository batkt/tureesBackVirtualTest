...
async function guilgeeUstgakhShalguur(req, res, next) {
    if (req.body.nevtersenAjiltniiToken && req.body.barilgiinId) {
        var ajiltan = await Ajiltan.findById(req.body.nevtersenAjiltniiToken.id);
        if (ajiltan.erkh == "Admin" || (ajiltan.tokhirgoo && ajiltan.tokhirgoo.guilgeeUstgakhErkh &&
            ajiltan.tokhirgoo.guilgeeUstgakhErkh.length > 0 && ajiltan.tokhirgoo.guilgeeUstgakhErkh.includes(req.body.barilgiinId))) {
            next();
        }
        else
            next(new Error("Энэ үйлдлийн эрхгүй байна!"));
