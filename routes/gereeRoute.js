...
router.route("/tulultTaniyaCGW").post(tokenShalgakh, async (req, res, next) => {
  await tulultTaniyaCGW(req.body.baiguullagiinId);
  res.send("Amjilttai");
});
...
const { gereeAshiglakhguiSaruud } = require("../controller/gereeController");
    ...
    const result = await Geree(req.body.tukhainBaaziinKholbolt).updateOne(
      ...
      },
      {
        $set: {
          ...
          "avlaga.guilgeenuud.$.bokhirUsDun": 0,
          "avlaga.guilgeenuud.$.usKhalaasanDun": 0,
          "avlaga.guilgeenuud.$.tsekhDun": 0,
          "avlaga.guilgeenuud.$.chadalDun": 0,
          "avlaga.guilgeenuud.$.sekhDemjikhTulburDun": 0,
        },
      },
    );
...
router.route("/suuliinZaaltAvya").get(tokenShalgakh, async (req, res, next) => {
  try {
    ...
    if (!suuliinZaalt) {
      ...
      if (lastExcel?.suuliinZaalt) {
        ...
        if (lastExcel.tooluuriinDugaar)
          tooluuriinDugaar = lastExcel.tooluuriinDugaar;
      }
    }

    console.log(
      `[suuliinZaaltAvya] Talbai: ${talbainDugaar}, Tailbar: ${tailbar}, GereeniiId: ${gereeniiId}, SuuliinZaalt: ${suuliinZaalt}, TooluuriinDugaar: ${tooluuriinDugaar}`,
    );

    res.send({
      suuliinZaalt,
      guilgeeniiId: suuliinGuilgeeniiId,
      gereeniiId: suuliinGereeniiId,
      tooluuriinDugaar,
    });
  } catch (err) {
    next(err);
  ...
  .get(tokenShalgakh, async (req, res, next) => {
    try {
      ...
      if (!baiguullagiinId || !barilgiinId) {
        return res
          .status(400)
          .send("baiguullagiinId болон barilgiinId шаардлагатай");
      }
...
router
  ...
  .get(tokenShalgakh, async (req, res, next) => {
    try {
      ...
      const existingGereenuud = await Geree(
        req.body.tukhainBaaziinKholbolt,
        false,
      )
        .find({
...
crud(
  ...
  async (req, res, next) => {
    try {
      ...
      var dugaarlalt = null;
      if (
        req.body.gereeniiDugaar &&
        req.body.gereeniiDugaar.match(/^ГД\d{6}(\d+)?$/)
      ) {
        ...
        req.body.gereeniiDugaar =
          basePrefix + String(maxDugaar).padStart(2, "0");
      }
  ...
  .post(tokenShalgakh, async (req, res, next) => {
    try {
      ...
      if (oldAvlaga) {
        ...
        if (!!ustgakhJagsaalt) {
          for (const key of ustgakhJagsaalt) {
    ...
    try {
      ...
      if (geree.avlaga?.guilgeenuud?.length > 0) {
        ...
        const monthEnd = new Date(
          moment(req.body.tsutslakhOgnoo).endOf("month"),
        );
        const duusakhOgnoo = new Date(
          moment(req.body.tsutslakhOgnoo).endOf("month"),
        );
        for (const g of geree.avlaga.guilgeenuud) {
          const ognoo = g.ognoo ? new Date(g.ognoo) : null;
          if (!ognoo || ognoo > duusakhOgnoo) continue;
          const turul = g.turul || "";
          const isBaritsaaOrAldangi =
            turul === "baritsaa" || turul === "aldangi";
          const tulukh = (g.tulukhDun || 0) - (g.khyamdral || 0);
          const uldegdel =
            (g.tulukhDun || 0) - (g.tulsunDun || 0) - (g.khyamdral || 0);
          if (
            isBaritsaaOrAldangi &&
            !(turul === "baritsaa" && (g.tulsunDun || 0) > 0)
          )
            continue;
          tsutsalsanUldegdel += uldegdel;
          if (
            ognoo >= monthStart &&
            ognoo <= monthEnd &&
            (g.tulukhDun || 0) > 0 &&
...
router
  ...
  .get(tokenShalgakh, async (req, res, next) => {
    try {
      ...
      if (!!body?.search) delete body.search;
      if (!body.query) body.query = {};
      const showTsutslagdsanAvlaga =
        body.query?.showTsutslagdsanAvlaga === true;
      ...
      khuudaslalt(Geree(req.body.tukhainBaaziinKholbolt, true), body)
        .then(async (result) => {
          ...
          if (showTsutslagdsanAvlaga) {
            ...
            const tsutslagdsanByContract = await Geree(
              req.body.tukhainBaaziinKholbolt,
              true,
            ).aggregate(tsutslagdsanQuery);
            const tsutslagdsanMapByGereeniiDugaar = {};
            const tsutslagdsanMapByRegister = {};
            (tsutslagdsanByContract || []).forEach((r) => {
              const avlaga =
                r.tsutslagdsanAvlaga > 0 ? r.tsutslagdsanAvlaga : 0;
              tsutslagdsanMapByGereeniiDugaar[r._id] = avlaga;
              const reg = r.register || "";
              tsutslagdsanMapByRegister[reg] =
                (tsutslagdsanMapByRegister[reg] || 0) + avlaga;
            });
            if (!result.jagsaalt) result.jagsaalt = [];
            result.jagsaalt.forEach((x) => {
            ...
            const cancelledWithPaymentsToAdd = Object.keys(
              cancelledWithPaymentsMap,
            );
            ...
            if (cancelledWithAvlaga.length > 0) {
              ...
              const cancelledContracts = await Geree(
                req.body.tukhainBaaziinKholbolt,
                true,
              )
                .find({
                  ...tsutslagdsanMatch,
                  gereeniiDugaar: { $in: cancelledGereeniiDugaar },
                })
                .lean();
              const avlagaMap = {};
              cancelledWithAvlaga.forEach((r) => {
                avlagaMap[r._id] = r.tsutslagdsanAvlaga;
              });
            ...
            const cancelledWithPaymentsToAddFiltered =
              cancelledWithPaymentsToAdd.filter(
  ...
  .post(tokenShalgakh, async (req, res, next) => {
    try {
      ...
      const hasTuluvluguut =
        gereenuud?.length > 0 && gereenuud[0]?.tuluvluguut?.length > 0;
      ...
      if (req.body.showTsutslagdsanAvlaga) {
        ...
        if (!!req.body.barilgiinId)
          tsutslagdsanMatch.barilgiinId = req.body.barilgiinId;
        const tsutslagdsanAvlagaQuery = [
          { $match: tsutslagdsanMatch },
          { $unwind: { path: "$avlaga.guilgeenuud" } },
          {
            $match: {
          ...
          { $match: { tsutslagdsanAvlaga: { $gt: 0 } } },
        ];
        const tsutslagdsanTuluvluguutQuery = [
          { $match: tsutslagdsanMatch },
          { $unwind: { path: "$avlaga.guilgeenuud" } },
          {
            $match: {
          ...
          { $match: { tuluvluguut: { $gt: 0 } } },
        ];
        const [tsutslagdsanAvlagaList, tsutslagdsanTuluvluguutList] =
          await Promise.all([
            Geree(req.body.tukhainBaaziinKholbolt, true).aggregate(
              tsutslagdsanAvlagaQuery,
            ),
            Geree(req.body.tukhainBaaziinKholbolt, true).aggregate(
              tsutslagdsanTuluvluguutQuery,
            ),
          ]);
        const cancelledWithAvlaga = new Set(
          (tsutslagdsanAvlagaList || []).map((r) => r._id),
        );
      ...
      body.query["gereeniiDugaar"] = { $in: turJagsaalt };
      body.query["tuluv"] =
        req.body.showTsutslagdsanAvlaga === true
          ? { $in: [1, -1] }
          : { $ne: -1 };
      body.lean = true;
      if (
        req.body.showTsutslagdsanAvlaga &&
        body.select &&
      ...
      if (req.body.showTsutslagdsanAvlaga && tsutslagdsanTuluvluguutList) {
        tsutslagdsanTuluvluguutList.forEach((r) => {
      ...
      if (req.body.showTsutslagdsanAvlaga && turJagsaalt.length > 0) {
        ...
        if (!!req.body.barilgiinId)
          storedMatch.barilgiinId = req.body.barilgiinId;
        ...
        .then(async (result) => {
          ...
          for (const id of turJagsaalt) {
            if (req.body.showTsutslagdsanAvlaga && allCancelledIds.has(id)) {
              ...
              const val =
                stored?.tuluvluguut ?? tuluvluguutMapForCancelled[id] ?? 0;
            ...
            if (relevantGereenuud.length > 0) {
              ...
              var suuliinGuilgee = allGuilgeenuud.filter((g_item) => {
                return (
                  ...
                  (!x.tooluuriinDugaar ||
                    g_item.tooluuriinDugaar == x.tooluuriinDugaar)
                );
              });
      ...
      if (niitGereenuud.length > 0) {
        for (const tukhainZardal of jagsaalt) {
          ...
          if (
            ...
            var umnukhZaalt = tukhainZardal.umnukhZaalt;

            if (umnukhZaalt === undefined || umnukhZaalt === null) {
          ...
          console.log(
            `[zaaltOlnoorOruulya] Talbai: ${tukhainZardal.talbainDugaar}, SuuliinZaalt: ${tukhainZardal.suuliinZaalt}, UmnukhZaalt: ${umnukhZaalt}, ZoruuDun: ${zoruuDun}, TempDun: ${tempDun}, BaiguullagaId: ${req.body.baiguullagiinId}`,
          );
          ...
          updateObject = {
            ...
            barilgiinId: req.body.barilgiinId,
            tailbar:
              ashiglaltiinZardal.ner +
              (req.body.nevtersenAjiltniiToken
                ? " Excel (" + req.body.nevtersenAjiltniiToken.ner + ")"
                : " Excel"),
        ...
        if (khariu?.length > 0) {
          for (const data of khariu) {
            ...
            if (filteredData?.length > 0) {
              for (const avlagaData of filteredData) {
            ...
            for (const geree of gereenuud) {
              if (geree.talbainIdnuud?.length > 0) {
                for (const talbainId of geree.talbainIdnuud) {
                  var khuuchinTalbai =
                    await Talbai(kholbolt).findById(talbainId);
                }
              }
            }
          }
...
router
  ...
  .post(tokenShalgakh, gereeAshiglakhguiSaruud);
...
router
  ...
  .post(tokenShalgakh, async (req, res, next) => {
    try {
      ...
      for (const geree of aldangiTulsunDunguud) {
        await Geree(req.body.tukhainBaaziinKholbolt).findByIdAndUpdate(
          { _id: geree?._id },
          {
...
router.route("/aldangiButsaakh").post(tokenShalgakh, async (req, res, next) => {
  try {
    ...
    if (aldangiinTuukh?.length > 0) {
      for (const aldangiData of aldangiinTuukh) {
