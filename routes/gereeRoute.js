    ...
    for await (const baiguullaga of baiguullaguud) {
      ...
      if(khariu?.length > 0)
      {
        for await (const data of khariu)  
        {
          ...
          var filteredTulsunDun = geree?.avlaga?.guilgeenuud.filter((data) => data.ognoo <= new Date(duusakhOgnoo) && data.ognoo >= new Date(ekhlekhOgnoo) && data.tulsunDun > 0).sort({ognoo: -1});
          ...
          if(filteredData?.length === 0 && tulsunDun >= 0 && filteredTulsunDun?.length > 0)
          {
            ...
            var khyamdral = {
              tulukhDun: 0,
              ognoo: filteredTulsunDun[0]?.ognoo,
              turul: "khungulult",
              khyamdral: khungulultiinDun,
