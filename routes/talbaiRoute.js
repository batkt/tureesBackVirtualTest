  ...
  try {
    ...
    const result = ekhniiData.map((e) => {
      ...
      return {
        ...
        niitKt: kt,
        etssiinUldegdel: ekh + dt - (kt - baritsaaPayments) + Math.max(0, (e.baritsaaAvakhDun || p.baritsaaAvakhDun || 0) - (baritsaaniiUldegdel || 0)),
    ...
    periodData.forEach((p) => {
      if (!ekhniiData.find((e) => e._id === p._id)) {
        ...
        result.push({
          ...
          niitKhyamdralAshiglalt: khyamdralAshiglalt,
          niitKt: kt,
          etssiinUldegdel: dt - (kt - baritsaaPayments) + Math.max(0, (p.baritsaaAvakhDun || 0) - (p.baritsaaniiUldegdel || 0)),
          tuluv: p.tuluv,
    ...
    canceledWithBalance.forEach((c) => {
      ...
      if (uld > 0 && !existingGereeniiDugaaruud.has(c.gereeniiDugaar)) {
        result.push({
          ...
          niitKt: 0,
          etssiinUldegdel: uld + Math.max(0, (c.baritsaaAvakhDun || 0) - (c.baritsaaniiUldegdel || 0)),
          tuluv: -1, 
