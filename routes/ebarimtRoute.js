    ...
    ebarimtDuudya(
      ...
      (d) => {
        try {
          ...
          BankniiGuilgee.findByIdAndUpdate(
            { _id: req.body.id },
    ...
    ebarimtButsaaya(
      ...
      async (d) => {
        ...
        if (butsaakhBarimt.guilgeeniiId)
          await BankniiGuilgee.findByIdAndUpdate(
...
router.post("/ebarimtToololtAvya", tokenShalgakh, async (req, res, next) => {
  try {
    ...
    var result = await Ebarimt.aggregate(query).catch((err) => {
      next(err);
    });
    ...
    var result1 = await BankniiGuilgee.aggregate(query).catch((err) => {
      next(err);
    });
