...
router.get("/zogsoolUilchluulegch", tokenShalgakh, async (req, res, next) => {
  try {
    ...
    console.log("📋 zogsoolUilchluulegch route called with query:", body.query);
    ...
    console.log(`🔍 Querying ${archiveName || "main collection"}...`);

    khuudaslalt(model, body)
      .then((result) => {
        console.log(`✅ Found ${result.jagsaalt?.length || 0} records`);
        res.send({
          ...result,
