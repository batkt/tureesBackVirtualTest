...
// Mock API for testing toki payment - Same as production but skips ebarimt/receipt generation
// Updates database with fake payment data for testing purposes
router.route("/v1/mockPay").post(async (req, res, next) => {
  try {
    let tulbur = [
      {
        ognoo: new Date(),
    ...
    var butsaakhKhariu = {
      success,
      message,
      isMockPayment: true, // Flag to indicate this is a mock/test payment
    };
    if (!tukhainObject) {
      res.send({ success: false, message: "Машины мэдээлэл олдсонгүй!" });
    } else {
