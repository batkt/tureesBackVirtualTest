...
router.post("/zogsoolSdkService", tokenShalgakh, async (req, res, next) => {
  try {
    ...
    try {
      if (
        khariu &&
        typeof khariu === "object" &&
        !khariu?.aldaa &&
        Array.isArray(khariu?.tuukh) &&
        khariu.tuukh.length > 0 &&
        Array.isArray(khariu?.tuukh?.[0]?.tsagiinTuukh) &&
        khariu.tuukh[0].tsagiinTuukh.length > 0 &&
        !!khariu.tuukh[0].tsagiinTuukh[0]?.garsanTsag
      ) {
        const t0 = khariu.tuukh[0];

        let zogsool = null;
        if (!!t0?.zogsooliinId) {
          zogsool = await Parking(req.body.tukhainBaaziinKholbolt).findById(
            t0.zogsooliinId
          );
        }

      
        if (!zogsool && !!req.body.baiguullagiinId) {
          zogsool = await Parking(req.body.tukhainBaaziinKholbolt).findOne({
            ...
            $or: [
              ...
              {
                "khaalga.camera": {
                  $elemMatch: { cameraIP: req.body.CAMERA_IP },
                },
              },
            ],
          });
        ...
        if (zogsool) {
          ...
          const niit = Number(bodsonDun || 0);
          const tulburSum = Array.isArray(t0?.tulbur)
            ? t0.tulbur.reduce((s, p) => s + Number(p?.dun || 0), 0)
            : 0;

          let tuluh = niit - tulburSum;
          if (!Number.isFinite(tuluh) || tuluh < 0) tuluh = 0;

          
          if (!t0.tulukhDun || Number(t0.tulukhDun) === 0) t0.tulukhDun = tuluh;
          if (!khariu.niitDun || Number(khariu.niitDun) === 0)
            khariu.niitDun = tuluh;

           
          if (!!khariu?._id && !!t0?.zogsooliinId) {
            await Uilchluulegch(req.body.tukhainBaaziinKholbolt).updateOne(
              { _id: khariu._id, "tuukh.0.zogsooliinId": t0.zogsooliinId },
              { $set: { "tuukh.0.tulukhDun": tuluh } }
            );
          }
        }
      }
